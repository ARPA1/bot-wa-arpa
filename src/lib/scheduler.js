...
function startDailyLimitReset(options = {}) {
    ...
    const scheduleReset = () => {
        ...
        logger.debug('Scheduler', `Daily limit reset scheduled in ${formatTimeRemaining(msUntilReset)}`);
...
function loadScheduledMessages(sock) {
    try {
        ...
        logger.debug('Scheduler', `Loaded ${savedTasks.length} scheduled messages`);
    } catch (error) {
...
function startGroupScheduleChecker(sock) {
    ...
    logger.debug('Scheduler', 'Group schedule checker started');
}
...
function startSewaChecker(sock) {
    ...
    const scheduleCheck = () => {
        ...
        logger.debug('Scheduler', `Sewa checker scheduled in ${formatTimeRemaining(msUntilMidnight)}`);
    ...
    scheduleCheck();
    logger.debug('Scheduler', 'Sewa checker enabled at 00:00');
}
