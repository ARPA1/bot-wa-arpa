...
function getGameContextInfo(title = '🎮 ZENITH GAMES', body = 'Have fun playing!') {
...
function getRpgContextInfo(title = '⚔️ ZENITH RPG', body = 'Adventure awaits!') {
    const saluranId = config.saluran?.id || '120363208449943317@newsletter'
    const saluranName = config.saluran?.name || config.bot?.name || 'Ourin-AI'
...
function createFakeQuoted(botName = 'Zenith-AI', verified = true) {
    return {
        key: {
            fromMe: false,
