...
async function handleSmartTriggers(m, sock, db) {
  ...
  try {
    ...
    const saluranName = config.saluran?.name || config.bot?.name || "Zenith-AI";
    const botName = config.bot?.name || "Zenith-AI";
...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
    ...
    if (!plugin) {
      ...
      // Add case commands to allCommands
      try {
        const { getCaseCommands } = require("../case/ourin");
        const caseCommands = getCaseCommands();
        for (const categoryCommands of Object.values(caseCommands)) {
